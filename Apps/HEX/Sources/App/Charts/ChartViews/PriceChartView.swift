...
struct PriceChartView: UIViewRepresentable {
    ...
    func updateUIView(_ combinedView: CombinedChartView, context _: Context) {
        ...
        let scaledOHLCV: [OHLCVData]
        
        switch chartScale {
        case .auto: scaledOHLCV = ohlcv
        case .log: scaledOHLCV = ohlcv.map{ OHLCVData(time: $0.time,
                                                      open: log10($0.open),
                                                      high: log10($0.high),
                                                      low: log10($0.low),
                                                      close: log10($0.close),
        ...
        switch chartType {
        case .line:
            combinedData.lineData = generateData(ohlcv: scaledOHLCV, color: .systemBlue)
            livePrice = LivePrice(price: scaledOHLCV.last?.close ?? 0.0,
                                  text: .label,
                                  background: .systemBlue)
        case .candlestick:
            combinedData.candleData = generateData(ohlcv: scaledOHLCV)
            combinedData.lineData = generateData(ohlcv: scaledOHLCV, color: .clear)
            livePrice = LivePrice(price: scaledOHLCV.last?.close ?? 0.0,
                                  text: .label,
                                  background: (scaledOHLCV.last?.close ?? 0.0 >= scaledOHLCV.last?.open ?? 0.0) ? .systemGreen : .systemRed)
        }
        combinedData.barData = generateData(ohlcv: scaledOHLCV)
        combinedView.data = combinedData
        
        if !ohlcv.isEmpty {
            combinedView.zoom(scaleX: 0, scaleY: 0, x: 0, y: 0)
            let pointsDisplayed = 32
            let xScale = Double(scaledOHLCV.count / pointsDisplayed)
