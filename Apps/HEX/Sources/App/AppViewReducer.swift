...
let appReducer = Reducer<AppState, AppAction, AppEnvironment> { state, action, environment in
    ...
    case .applicationDidFinishLaunching:
        do {
            try environment.dbQueue.write { db in
                try db.create(table: "ohlcvTimeScaleData", options: [.ifNotExists]) { t in
    ...
    case .loadFromDisk:
        do {
            state.ohlcv = try environment.dbQueue
                .read { db in
                    try OHLCVTimeScaleData
                        .filter(Column("code") == state.selectedTimeScale.code)
                        .fetchAll(db)
                }
                .map({ OHLCVData(ohlcvTimeScale: $0) })
        } catch {
            os_log("%@", log: .hexApp, type: .error, error.localizedDescription)
        }
        return .none
    ...
    case let .updateChart(result):
        let code = state.selectedTimeScale.code
        switch result {
        case let .success(chartData):
            do {
                try environment.dbQueue.write { db in
                    ...
                    try chartData.forEach { data in
                        try OHLCVTimeScaleData(ohlcv: data, code: code).insert(db)
                    }
                }
            } catch {
                print(error)
